...
/* $Id: schema.sql,v 1.20 2009/10/27 20:55:34 akotov2 Exp $ */

/* If database already exists, we delete it */
DROP DATABASE IF EXISTS quse_debug;

/* Create the database */
CREATE DATABASE quse_debug;

/* Deleting the user, if he existed */
DELETE FROM mysql.user WHERE User='quse_debug';

DROP USER 'quse_debug'@'localhost';

DROP USER 'quse_debug'@'%';

/* Create a user with access to the DB from the local machine */
CREATE USER 'quse_debug'@'localhost' IDENTIFIED BY 'TIManUIUC';

/* Create a user with access to the DB from the network */
CREATE USER 'quse_debug'@'%' IDENTIFIED BY 'TIManUIUC';

/* Giving the newly created user all privileges for the database */
GRANT ALL ON quse_debug.* TO 'quse_debug';
